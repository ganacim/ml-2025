            ...
            for epoch in pbar:
                ...
                for b, (X_train, Y_train) in enumerate(pbar_train):
                    ...
                    board.log_scalars(
                        "Curves/Loss_Batch",
                        {
                            "Discriminator_Loss": (Le / len(X_train)),
                            "Generator_Loss": (Lg / len(X_train)),
                            "Lae": (Lae / len(X_train)),
                    #         "D(x)": d_x / len(X_train),
                ...
                pbar.set_postfix(
                    {
                        "Loss_d": Loss_d.item() / len(train_data_loader.dataset),
                        "Loss_g": Loss_g.item() / len(train_data_loader.dataset),
                        "Lae": Total_Lae.item() / len(train_data_loader.dataset),
                        # "m": model.m
                    }
                )
                ...
                board.log_scalars(
                    "Curves/Loss_Epoch",
                    {
                        "Discriminator_Loss": (Loss_d.item()/ len(train_data_loader.dataset)),
                        "Generator_Loss": (Loss_g.item()/ len(train_data_loader.dataset)),
                        "Lae": (Total_Lae.item() / len(train_data_loader.dataset)),
                #         "D(x)": D_x,
                #         "DG(z)_1": DG_z1,
                #         "DG(z)_2": DG_z2,
