...
class Train(Base):
    ...
    def run(self):
        ...
        if (model.name() == 'VAE_GAN') or (model.name() == 'final_VAE_GAN'):
            ...
            discriminator_optimizer = model.get_discriminator_optimizer(learning_rate=self.learning_rate)
            ...
            try:  # let's catch keyboard interrupt
                ...
                for epoch in pbar:
                    ...
                    for X_train, Y_train in pbar_train:
                        ...
                        optimizer.zero_grad()
                        X_recon = model(X_train)
                        loss, rec_loss, kl_loss, adv_loss = model.evaluate_loss(X_train, X_recon)
                        loss.backward()
                        optimizer.step()
                        ...
                        train_loss = model.evaluate_loss(Y_train_pred, Y_train)[0]
                        train_loss.backward()
                        optimizer.step()
