...
class CelebAAlignSplit(Base):
    ...
    def run(self):
        ...
        files = list((data_path("celeba_align") / "data").glob("*.jpg"))
        ...
        files = [f.relative_to(data_path("celeba_align")) for f in files]
        ...
        # save files
        with open(data_path("celeba_align") / "train.txt", "w") as f:
            f.write("\n".join([str(f) for f in train_files]))

        with open(data_path("celeba_align") / "validation.txt", "w") as f:
            f.write("\n".join([str(f) for f in val_files]))

        with open(data_path("celeba_align") / "test.txt", "w") as f:
